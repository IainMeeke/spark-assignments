...
object WikipediaRanking {
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(art => langs.filter(art.mentionsLanguage).map(lang => (lang,art))).groupByKey()
  ...
  val a: RDD[(String, Int)] = rdd.flatMap(art => langs.filter(art.mentionsLanguage).map(lang => (lang,1)))
    a.reduceByKey(_ + _).collect.toList.sortBy(_._2)(Ordering[Int].reverse)
  }
